...
function extractRhyme(pinyinStr: string) {
...
function getRhymeForChineseChar(chineseChar: string) {
  const pinyinResult = pinyin(chineseChar, { style: pinyin.STYLE_NORMAL });
...
function checkIfRhymes(char1: string, char2: string) {
...
function analyzeRhymes(text: string) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (line1.length < 1 || line2.length < 1) continue;
...
async function rhymeTest(message: Message) {
