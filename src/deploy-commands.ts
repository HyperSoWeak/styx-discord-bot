...
const deployToGuild =
  args.includes("guild") ||
  (AppConfig.isDevelopment && !args.includes("global"));
...
console.log(
  chalk.cyan(
    `Deploy target: ${deployToGuild ? "Guild (" + AppConfig.TEST_GUILD_ID + ")" : "Global"}`,
  ),
);
  ...
  try {
    ...
    if (deployToGuild) {
      if (!AppConfig.TEST_GUILD_ID) {
        throw new Error(
          "TEST_GUILD_ID is not defined in environment variables.",
        );
      }
      data = (await rest.put(
        Routes.applicationGuildCommands(
          AppConfig.CLIENT_ID,
          AppConfig.TEST_GUILD_ID,
        ),
        {
          body: commands,
        },
      ...
      console.log(
        chalk.green(
          `Successfully reloaded ${data.length} application (/) commands to guild.`,
        ),
      );
    } else {
